using IGamingTest.Infrastructure.Hangfire.CronExpressionGenerator.Enums;
using IGamingTest.Infrastructure.Hangfire.CronExpressionGenerator.Resources;
using IGamingTest.Infrastructure.Hangfire.CronExpressionGenerator.Utils;
using System.Globalization;

namespace IGamingTest.Infrastructure.Hangfire.CronExpressionGenerator;
...
public class CronGenerator
{
    ...
    public static string GenerateSetDayCronExpression(TimeSpan runTime, DayOfWeek dayToRun)
        => $"{ParseCronTimeSpan(runTime)} * {ParseDayOfWeek((int)dayToRun)}";
    ...
    public static string GenerateSetDayCronExpression(TimeSpan runTime, int dayToRun)
        => $"{ParseCronTimeSpan(runTime)} * {ParseDayOfWeek(dayToRun)}";
    ...
    public static string GenerateWeekDayOnlyCronExpression(TimeSpan runTime)
        => $"{ParseCronTimeSpan(runTime)} * MON-FRI";
    ...
    public static string GenerateMultiDayCronExpression(TimeSpan runTime, List<int> daysToRun)
        => CreateMultiDayValue(runTime, daysToRun);
    ...
    public static string GenerateSetDayMonthlyCronExpression(TimeSpan runTime, TimeOfMonthToRun timeOfMonthToRun, DayOfWeek dayToRun, int amountOfMonthsToRunAfter)
        => CreateTimeOfMonthValue(runTime, timeOfMonthToRun, (int)dayToRun, amountOfMonthsToRunAfter);
    ...
    public static string GenerateSetDayMonthlyCronExpression(TimeSpan runTime, TimeOfMonthToRun timeOfMonthToRun, int dayToRun, int amountOfMonthsToRunAfter)
        => CreateTimeOfMonthValue(runTime, timeOfMonthToRun, dayToRun, amountOfMonthsToRunAfter);
    ...
    public static string GenerateYearlyCronExpression(TimeSpan runTime, int dayOfMonthToRunOn, MonthOfYear monthToRunOn)
        => CreateYearlyValue(runTime, dayOfMonthToRunOn, (int)monthToRunOn);
    ...
    public static string GenerateYearlyCronExpression(TimeSpan runTime, int dayOfMonthToRunOn, int monthToRunOn)
        => CreateYearlyValue(runTime, dayOfMonthToRunOn, monthToRunOn);
    ...
    private static string CreateTimeOfMonthValue(TimeSpan runTime, TimeOfMonthToRun timeOfMonthToRun, int dayToRun, int amountOfMonthsToRunAfter)
    {
        ...
        if (timeOfMonthToRun != TimeOfMonthToRun.Last)
        {
            ...
            var timeOfMonthToRunIntValue = (int)timeOfMonthToRun;
            return $"{ParseCronTimeSpan(runTime)} * 1/{amountOfMonthsToRunAfter} {day}#{timeOfMonthToRunIntValue}";
        }
        else
        {
            var day = dayToRun + 1;
    ...
    {
        var minutes = RemoveExtraDigitFromTimeValue(timeSpan.ToString("mm", CultureInfo.InvariantCulture));
        var hours = RemoveExtraDigitFromTimeValue(timeSpan.ToString("hh", CultureInfo.InvariantCulture));
    ...
    private static string RemoveExtraDigitFromTimeValue(string timeValue)
        ...
        => timeValue[0] == '0' ? timeValue[1].ToString() : timeValue;
    ...
    private static string ParseDayOfWeek(int dayOfTheWeek)
        => dayOfTheWeek switch
        {
            ...
            5 => "FRI",
            6 => "SAT",
            _ => throw new ArgumentOutOfRangeException(nameof(dayOfTheWeek), dayOfTheWeek,
                ExceptionMessages.InvalidDayOfTheWeekParameterException)
        };
